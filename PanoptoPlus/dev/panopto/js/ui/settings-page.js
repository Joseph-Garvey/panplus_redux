...
let SettingsPage = (() => {
  ...
  class SettingsPage extends preact.Component {
    constructor() {
      super();
      // Add state for the download status message
      this.state = {
        vttDownloadStatus: '',
    ...
    /**
     * Download the VTT transcript file using message passing.
     */
    async downloadVtt() {
      console.log("Download VTT button clicked on settings page");
      this.setState({ vttDownloadStatus: 'Finding active Panopto tab...', isDownloadingVtt: true });

      try {
          // 1. Find active Panopto tab
          const tabs = await new Promise((resolve) => {
              chrome.tabs.query({
                  active: true,
                  currentWindow: true,
                  url: [ // Ensure it's a Panopto viewer page
                      "*://*.panopto.com/Panopto/Pages/Viewer.aspx*",
                      "*://*.panopto.eu/Panopto/Pages/Viewer.aspx*"
                      // Add other Panopto domains if necessary
                  ]
              }, resolve);
          });

          if (!tabs || tabs.length === 0) {
              throw new Error("No active Panopto viewer tab found. Please navigate to the video page first.");
          }
          const tabId = tabs[0].id;
          this.setState({ vttDownloadStatus: 'Requesting VTT data from Panopto page...' });

          // 2. Send message to content script on that tab
          const response = await new Promise((resolve, reject) => {
              // Set a timeout for the response
              const timeoutId = setTimeout(() => {
                  reject(new Error("Request timed out. Content script might be busy or unresponsive. Try refreshing the Panopto page."));
              }, 15000); // 15 second timeout

              chrome.tabs.sendMessage(tabId, { type: "GET_VTT_DATA" }, (response) => {
                  clearTimeout(timeoutId); // Clear the timeout if response received
                  if (chrome.runtime.lastError) {
                      reject(new Error(`Could not communicate with Panopto page: ${chrome.runtime.lastError.message}. Try refreshing the Panopto page.`));
                  } else if (response && response.success) {
                      resolve(response);
                  } else if (response && !response.success) {
                      reject(new Error(response.error || "Content script failed to get VTT data."));
                  } else {
                      // This case might happen if the content script doesn't send any response
                      reject(new Error("Received no valid response from Panopto page. Ensure the extension is active and the page is fully loaded."));
                  }
              });
          });

          // 3. Process response and trigger download
          this.setState({ vttDownloadStatus: 'VTT data received. Preparing download...' });
          const { rawVtt, deliveryId } = response;

          if (!rawVtt || !deliveryId) {
               throw new Error("Received incomplete VTT data from content script.");
          }

          const blob = new Blob([rawVtt], { type: 'text/vtt;charset=utf-8' });
          const downloadUrl = URL.createObjectURL(blob);
          const filename = `panopto_${deliveryId}.vtt`;
          ...
          chrome.downloads.download({
              ...
              if (chrome.runtime.lastError) {
                  ...
                  this.setState({ vttDownloadStatus: `Download failed: ${chrome.runtime.lastError.message}`, isDownloadingVtt: false });
              } else {
                  console.log(`Download started with ID: ${downloadId}`);
                  this.setState({ vttDownloadStatus: `Download started: ${filename}`, isDownloadingVtt: false });
              }
              setTimeout(() => URL.revokeObjectURL(downloadUrl), 1000); // Clean up blob URL
          });

      } catch (error) {
          console.error("Error downloading VTT:", error);
          this.setState({ vttDownloadStatus: `Error: ${error.message}`, isDownloadingVtt: false });
      }
    }

    render({}, { vttDownloadStatus, isDownloadingVtt }) { // Destructure state here
      let index = null;
      let form = document.getElementById("settings-form");
      ...
      const extensionVersion = "1.4.11r4"; // Extracted from manifest.json

      // Find the existing download button definition and add the status div after it
      const downloadButton = preact.h("div", {
        // Disable button while processing
        onClick: !isDownloadingVtt ? this.downloadVtt.bind(this) : null,
        class: `ui-state-default ui-button save-btns ${isDownloadingVtt ? 'ui-state-disabled' : ''}`, // Add disabled class
        style: "margin-top: 10px;"
      }, preact.h("a", {
        href: "#",
        class: "ui-tabs-text-lookalike"
      }, preact.h("span", null, preact.h("i", { class: "fas fa-download", style: "padding-right: 4px;" }), "Download Transcript (.vtt)")));

      // Add the status display div
      const downloadStatusDiv = preact.h("div", {
          id: "vttDownloadStatus", // Add an ID if needed
          style: "margin-top: 5px; font-size: 0.9em; color: grey; min-height: 1.2em;" // Basic styling
      }, vttDownloadStatus); // Display status from state

      return preact.h("form", {
        id: "settings-form"
      }, 
      // ... existing preact.h calls ...
      preact.h("hr", null), 
      preact.h("h4", null, "Bug Report"), 
      preact.h("div", null, 
        preact.h("a", {
      ...
      preact.h("div", null, `Version: ${extensionVersion}`), // Add version display here
      preact.h("br", null), 
      preact.h("hr", null), 
      preact.h("div", {
      ...
      downloadButton, // Include the download button
      downloadStatusDiv, // Include the status div right after the button
      preact.h("div", {
        ...
        href: "#",
        class: "ui-tabs-text-lookalike"
      }, preact.h("span", null, "Make Panopto Full-screen"))));
    }
